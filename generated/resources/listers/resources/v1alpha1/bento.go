...
// bentoLister implements the BentoLister interface.
type bentoLister struct {
	indexer cache.Indexer
}

// NewBentoLister returns a new BentoLister.
func NewBentoLister(indexer cache.Indexer) BentoLister {
...
type BentoNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.Bento, error)
	BentoNamespaceListerExpansion
}

// bentoNamespaceLister implements the BentoNamespaceLister
// interface.
type bentoNamespaceLister struct {
	indexer   cache.Indexer
	namespace string
}
...
// Get retrieves the Bento from the indexer for a given namespace and name.
func (s bentoNamespaceLister) Get(name string) (*v1alpha1.Bento, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("bento"), name)
	}
	return obj.(*v1alpha1.Bento), nil
}
